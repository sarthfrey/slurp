from slurp3 import correct, known_word
...
import time, numpy, json

with open('unigram.json', 'r') as f:
	BAG_OF_WORDS = json.load(f)

def spell_test(tests, alpha, beta, bias=0):
	num_accurate, num_total, num_unknown, start = 0, 0, 0, time.clock()
	if bias:
		for target in tests:
			if target in BAG_OF_WORDS:
				BAG_OF_WORDS[target] += bias
			else:
				BAG_OF_WORDS[target] = bias
	for correct_word, error_words in tests.items():
		for error_word in error_words.split():
			corrected_word = correct(error_word, BAG_OF_WORDS, alpha, beta)
			if corrected_word == correct_word:
				num_accurate += 1
			else:
				print error_word + ' | ' + corrected_word + ' | ' + correct_word
			if not known_word(corrected_word):
				num_unknown += 1
			num_total += 1
	accuracy = str(float(num_accurate) / num_total * 100)
	seconds = time.clock() - start
	rate =  num_total / seconds
	print dict(accuracy=accuracy, total= num_total, seconds=seconds, rate=rate, unknown=num_unknown)
	return dict(accuracy=accuracy, alpha=alpha, beta=beta)

def alpha_test(alphas, beta):
	best_performance = dict(accuracy=0, alpha=None)
	performances= []
	for alpha in alphas:
		performance = spell_test(alpha, beta)
...
def beta_test(alpha, betas):
	best_performance = dict(accuracy=0, beta=None)
	performances= []
	for beta in betas:
		performance = spell_test(alpha, beta)
		if performance['accuracy'] > best_performance['accuracy']:
...
#alpha_test(numpy.linspace(0.5, 1.5, num=10), 1)
#beta_test(1.16, numpy.linspace(0.1, 1, num=10))
print spell_test(tests1, 1.16, 1, 10000)
